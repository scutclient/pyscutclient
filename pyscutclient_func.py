#!/usr/bin/python
#coding=utf8



#把一个'0'~'255'的字符串或数字转换成十六进制的char 如 '49'->'\x31'  49->'\x31'
def strtohexchar(s):
	return chr(int(s))


def encode(base):
	result = 0
	for i in range(8):
		result <<= 1
		result |= base & 0x01
		base >>= 1
        r = ~result
        if r<0:
        	r += 256   #在python中执行  ～1会返回-2，应加256（因为不是unsigned char）
        return r


def gen_checksum(buf):
	table = ()
	
	for i in range(21):
		index = buf[21] ^ buf[i]
		buf[21] = buf[22] ^ table[index*2+1]
		buf[22] = table[index*2]
	for i in range(23):
		buf[i] = encode(buf[i])
	
	
	returnstr = ''
	for s in buf:
		returnstr += strtohexchar(s)
	return returnstr  #最终返回字符串 '\x12\x34\x5f' 共23字节

